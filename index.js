...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var item = document.getElementById("far");

    item.addEventListener("mouseover", func2, false);
    item.addEventListener("mouseout", func3, false);
...
function clear()
{
    document.getElementById('brow-input').value = '';
    document.getElementById('brow-input-2').value = '';
...
function convert()
{
    ...
    myHeaders.append("apikey", "HoXcMnSGBRK0K5WNdlkXu7KAwjXXCQxh");
    ...
    var currency = document.getElementById('brow-input').value;
    var exchange_to = document.getElementById('brow-input-2').value;
    
    currency.toUpperCase();
    exchange_to.toUpperCase();
    var amount = document.querySelector('#amount').value;
    amount = parseInt(amount);

    currency = currency.slice(0, 3);
    exchange_to = exchange_to.slice(0, 3);


        fetch(`https://api.apilayer.com/exchangerates_data/latest?symbols=${exchange_to}&base=${currency}`, requestOptions)
        .then(response => response.json())
        .then(result => {

            let rates = result.rates[exchange_to];
            rates = rates*amount;
            console.log(rates);
            var h1 = document.querySelector('#h1');
            var subresult = document.querySelector('#subresult');
            ...
            if (rates !== undefined) {
                ...
                if (amount > 1)
                {
                    h1.innerHTML = `${amount} ${currency} are equal to ${rates.toFixed(3)} ${exchange_to}.`;
                    subresult.innerHTML = `${amount} ${currency} = ${rates.toFixed(3)} ${exchange_to}`;
                    rates = 0
                }
                else
                {
                    h1.innerHTML = `${amount} ${currency} is equal to ${rates.toFixed(3)} ${exchange_to}.`;
                    subresult.innerHTML = `${amount} ${currency} = ${rates.toFixed(3)} ${exchange_to}`;
                    rates = 0
                }
...
function get_currencies(countries)
{
    for(var i = 0; i < countries.length; i++)
    {
        ...
        option.value = countries[i].cc + ' - ' + countries[i].name;
        var brow = document.querySelector('#brow');
        brow.appendChild(option);
    }
}
